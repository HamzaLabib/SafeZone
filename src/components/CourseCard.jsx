...
export function CourseCard({ course }) {
  return (
    ...
    >
      <img src={course.image} className="h-40 w-full object-cover" alt="" onError={useLogoFallback} />
      <div className="flex flex-1 flex-col p-5">
        ...
        <p className="mt-2 flex flex-wrap gap-4 text-sm text-slate-600">
          <span className="inline-flex items-center gap-1">
            <Clock3 className="h-4 w-4 text-academyBlue" />
            {course.duration}
          </span>
          <span className="inline-flex items-center gap-1">
            <BookOpen className="h-4 w-4 text-academyBlue" />
            {course.level}
          </span>
        </p>
